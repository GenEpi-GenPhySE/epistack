...
Epistack is a visualisation package. It uses as input GRanges objects with
...
the visualisation functions using the example dataset included in the dataset.

The dataset can be access with:
...
The second parameter, `patterns` specify which columns of `mcols(gr)` 
should be displayed as heatmaps. The `patterns` values are prefixes or
...
If a `bin` column is present, it is used to generat one average profile per bin.
...
# Building epistack input object
