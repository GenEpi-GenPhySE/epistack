...
```{r ready_to_plot}
mcols(merged_peaks) <- cbind(mcols(merged_peaks), coverage_matrices[[1]], coverage_matrices[[2]], coverage_matrices[[3]])
```
...
plotEpistack(
    ...
    tints = c("blue", "red", "grey"), 
    titles = c("Rep1", "Rep2" , "Input"),
    x_labels = xlabs,
    zlim = c(0, 2), ylim = c(0, 3), 
...
plotEpistack(
    ...
    metric_col = "mean_scores", metric_title = "Peak score", metric_label = "log10(score)",
    metric_transfunc = function(x) log10(x),
    bin_palette = colorRampPalette(c("black", "blue", "red"))
)
